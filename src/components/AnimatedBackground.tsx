 ...
 * Particle class
 * 
 * Represents a single particle in the animated background.
 */
class Particle {
  x: number;
  y: number;
  ...
  constructor(canvas: HTMLCanvasElement, isScrolling: boolean) {
  ...
  update(canvas: HTMLCanvasElement, isScrolling: boolean) {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      resizeTimeout = setTimeout(() => {
        ...
        particlesRef.current = createParticles(40, canvas);
      }, 100);
    };
    ...
    const createParticles = (count: number, canvas: HTMLCanvasElement) => {
      ...
      for (let i = 0; i < count; i++) {
        particles.push(new Particle(canvas, isScrolling));
      }
      return particles;
    };
    ...
    function animate() {
      if (!ctx || !canvas) return; // Add null check for ctx and canvas
      ...
      for (const particle of particles) {
        particle.update(canvas, isScrolling);
        particle.draw(ctx);
      }
